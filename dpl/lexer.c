...
lexeme *lexNumber(FILE *f){
	...
	while(!feof(f) && (isdigit(ch) || ch == '.')){
		...
		if(ch == '.' && real == 1){
			...
			fprintf(stdout, "SYNTAX ERROR: BAD NUMBER AT LINE %d!\n", LineNumber);
			exit(-1);
		}
		if(ch == '.') real = 1;
		ch = readChar(f);
    ...
    switch(ch){ 
        ...
        default: 
            ...
            if (isdigit(ch)) { 
                ...
                fprintf(stdout, "Syntax error: unknown lexical %c at line %d!\n", ch, LineNumber);
                exit(-1);
            }
    }
...
bool check(char *type){ 
    ...
    if(/*strcmp(getLexemeType(token),type) == 0*/ getLexemeType(token) == type){
...
void matchNoAdvance(char *type){
    if (!check(type)){
        fprintf(stdout, "Syntax error: looking for %s found %s on line %d\n",type, getLexemeType(token), LineNumber);
        matchError = 1;
        exit(-1);
    }
}
